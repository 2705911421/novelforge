(function () {
  ...
  function drawPlotGraph() {
    ...
    svg.addEventListener('wheel', event => { event.preventDefault(); scale = Math.max(.35, Math.min(2.4, scale * (event.deltaY < 0 ? 1.1 : .9))); update(); }, { passive: false });
    svg.addEventListener('pointerdown', event => { const element = event.target.closest('[data-plot-node]'); if (element) { const node = allNodes.find(item => item.id === element.dataset.plotNode); if (node) { plotState.selectedId = node.id; renderPlotInspector(); dragged = { node, point: plotScenePoint(event, scene), moved: false }; svg.setPointerCapture(event.pointerId); } return; } panning = true; panX = event.clientX - tx; panY = event.clientY - ty; svg.setPointerCapture(event.pointerId); });
    svg.addEventListener('pointermove', event => { if (dragged) { const point = plotScenePoint(event, scene); const dx = point.x - dragged.point.x, dy = point.y - dragged.point.y; if (Math.abs(dx) + Math.abs(dy) > 2) dragged.moved = true; dragged.node.x = Math.round(dragged.node.x + dx); dragged.node.y = Math.round(dragged.node.y + dy); dragged.point = point; const nodeElement = [...svg.querySelectorAll('[data-plot-node]')].find(item => item.dataset.plotNode === dragged.node.id); if (nodeElement) nodeElement.setAttribute('transform', `translate(${dragged.node.x},${dragged.node.y})`); return; } if (panning) { tx = event.clientX - panX; ty = event.clientY - panY; update(); } });
    svg.addEventListener('pointerup', () => { if (dragged) { const item = dragged; dragged = null; if (item.moved) savePlotDelta({ operations: [{ op: 'move_node', id: item.node.id, x: item.node.x, y: item.node.y }] }); } panning = false; });
    svg.querySelectorAll('[data-plot-node]').forEach(element => element.addEventListener('click', () => { plotState.selectedId = element.dataset.plotNode; renderPlotInspector(); drawPlotGraph(); })); update();
  }
  ...
  PAGES.thought = async (p) => {
    if (!S.book) return go('dashboard');
    try {
      const session=await api('GET',`/books/${S.book}/thought-session?optional=true`); if(session.exists===false){ return renderFeatureEmpty(p,'念头创作','还没有建立念头访谈。先从一个冲突、人物或画面开始，系统会逐步追问并生成可确认的 Story Bible。','<button class="btn btn-primary" onclick="go(\'create\')">新建念头作品</button>'); } const turns=session.turns||[];
      p.innerHTML=header('念头创作', '只从一个念头开始，AI 会持续追问，直到形成可审阅的完整小说框架', `<button class="btn btn-secondary" onclick="go('wizard')">查看 Story Bible</button><button class="btn btn-primary" onclick="generateThoughtFramework()">生成完整框架</button>`) + `<div class="content"><div class="grid grid-2"><div><div class="card"><div class="card-title-row"><h3>原始念头</h3><span class="badge badge-info">${esc(session.status||'questioning')}</span></div><p style="white-space:pre-wrap;line-height:1.8">${esc(session.seed||'—')}</p></div><div class="card"><h3>访谈记录</h3><div>${turns.map(turn => `<div class="list-row" style="align-items:flex-start"><span class="badge ${turn.role==='assistant'?'badge-info':'badge-success'}">${turn.role==='assistant'?'AI':'你'}</span><span style="white-space:pre-wrap;line-height:1.7">${esc(turn.content||'')}</span></div>`).join('')}</div></div></div><div><div class="card"><h3>继续回答</h3><p class="dim-note">问题 ${Number(session.question_index||0)+1} · 每次只回答当前问题即可。</p><div class="card mt16" style="background:var(--bg)"><p style="white-space:pre-wrap;line-height:1.8">${esc(session.current_question||'请继续补充这个故事。')}</p></div><textarea class="input textarea" id="thought-answer" placeholder="想到什么就写什么，不需要先整理成大纲"></textarea><button class="btn btn-primary mt8" onclick="submitThoughtAnswer()">回答并继续追问</button>${session.error?`<div class="warn-banner mt16">上次任务失败：${esc(session.error)}。可以修正模型配置后重新回答。</div>`:''}</div><div id="thought-task-state"></div><div class="card"><h3>使用说明</h3><p class="dim-note" style="line-height:1.8">生成后的 25 步框架会进入 Story Bible 草稿，仍需你确认。念头访谈和自动框架不会直接生成章节。</p></div></div></div></div>`;
